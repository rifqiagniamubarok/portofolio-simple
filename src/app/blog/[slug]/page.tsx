...
import { getDetailBlog, viewBlog } from '@/server/blog';
...
const mdxFiles = [
  {
    slug: 'how-to-install-prisma-in-nextjs',
    file: Blogs.HowToInstallPrismaInNextJs,
  },
];
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const project = await getDetailBlog(slug);
      ...
      <>
        <div className="w-fit max-w-[634px] mb-10 pt-5 space-y-4">
          ...
          <div className="w-full rounded-sm aspect-video relative overflow-hidden">
            {project?.thumbnail && <Image src={project?.thumbnail} alt="signlys" fill className="object-cover object-top" />}
